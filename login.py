...
class Login():
    def __init__(self, root, gif_fps=15):
        ...
        # Load the 'right' sound.
        self.sound_right = mixer.Sound("assets\\audio\\right.mp3")

        # Define a mapping for frame delay based on the desired FPS for GIF
        # animation.
        self.fps = {10: 100, 15: 67, 24: 42, 30: 33, 36: 28, 42: 24}
        # Path to the CAPTCHA image.
        self.captcha = "assets\\images\\captcha.png"
        # Path to the GIF animation.
        ...
        self.main.geometry(
            f"{window_width}x{window_height}+{int(pos_x)}+{int(pos_y)}")
        ...
        self.picture_label = tk.Label(
            self.picture_frame, image=self.picture).pack()
    ...
    def display_gif(self):
        ...
        frames = [
            tk.PhotoImage(
                file=self.gif_file,
                format=f'gif -index {i}') for i in range(frameCnt)]
